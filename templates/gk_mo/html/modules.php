	...
	if (!empty ($module->content)) {		
		...
		if($module->showtitle) {	
			if($params->get('module_link_switch')) {
				echo '<h'.$header_type.' class="header"><a href="'. $params->get('module_link') .'">'. preg_replace('/__(.*?)__/i', '<small>${1}</small>', $module->title) .'</a></h'.$header_type.'>';
			} else {
				echo '<h'.$header_type.' class="header">'. preg_replace('/__(.*?)__/i', '<small>${1}</small>', $module->title) .'</h'.$header_type.'>';
			}
		}
