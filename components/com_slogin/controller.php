...
class SLoginController extends SLoginControllerParent
{
    ...
    public function auth()
    {
        ...
        $redirect = JURI::base().'component/slogin/callback/'.$plugin;
    ...
    public function check()
    {
        ...
        $plugin = $input->getString('plugin', '');
        if(JRequest::getVar('vk_token', null))$plugin="vkontakte";
        //echo $plugin;
...
//        $db->setQuery($query)->execute();
    ...
    private function storeSloginUser($user_id, $slogin_id, $provider){
        ...
        JTable::addIncludePath(JPATH_ROOT.DS.'components/com_slogin/tables');
    ...
    protected function storeOrLogin($popup=false)
    {
        ...
        if (!$sloginUserId){
            ...
            $isVk=JRequest::getVar('vk_token', null);
            if($joomlaUserId > 0)
            {
                $data = array(
                    'email' => $this->email,
                ...
                {
                    if(!$isVk)$return = base64_decode($model->getReturnURL($this->config, 'after_reg_redirect'));
                }
        ...
        $isVk=JRequest::getVar('vk_token', null);
        if(!$isVk)$this->displayRedirect($return, $popup, $msg);
        //else echo "sss";
    }
