     ...
     block.edit=function(id){
         ...
         if(marker!=null){
            ...
            marker.infoWindow.setContent(content[0]);
             ...
             noticeBar.notice({child: block.editForm});
...
function AddButton(type, main, list, data){
    ...
    addFunctions['meet_2']=function(data){
         ...
         noticeBar.close();
         var form=new WXForm({title: "Новая встреча с другом "+data.info.name});
         form.attr({class: "innerItem"})
         form.addString({id: "name", name: "Название встречи"});
         form.addString({id: "description", name: "Описание встречи"});
         form.addString({id: "distance", name: "Расстояние оповещения"});
         form.addHidden({id: "people_id", value: data.id});
         form.addActions([{id: "complete", name: "Добавить", actionType: "add", action: "meet", callback: function(){
             main.update.bind(main)();
             noticeBar.close();
         }}, {id: "cancel", name: "Отмена", callback: noticeBar.close.bind(noticeBar)}]);
         noticeBar.notice({type: "meet", child: form});
    }
    ...
    addFunctions['task_2']=function(data){
         ...
         noticeBar.close();
         var form=new WXForm({title: "Новая задача в "+data.info.name});
         form.attr({class: "innerItem"})
         form.addString({id: "name", name: "Название задачи"});
         form.addString({id: "description", name: "Описание задачи"});
         form.addString({id: "distance", name: "Расстояние оповещения"});
         form.addHidden({id: "place_id", value: data.id});
         form.addActions([{id: "complete", name: "Добавить", actionType: "add", action: "anchor", callback: function(){
             main.update.bind(main)();
             noticeBar.close();
         }}, {id: "cancel", name: "Отмена", callback: noticeBar.close.bind(noticeBar)}]);
         noticeBar.notice({type: "task", child: form});
    }
